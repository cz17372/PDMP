...
function loss(tau;J0,End,y)
    preindex = findall(J0.τ[end] .< y.t .< tau)
    proindex = findall(tau .< y.t .< End)
    l = sum((y.y[preindex] .- J0.ϕ[end]).^2) + sum((y.y[proindex] .- mean(y.y[proindex])).^2)
    return l
end
function GenZ(J0,t0,t1,t2,y,par,auxpar)
    ...
    if M == 1
        ...
        posttau = K==0 ? t2 : τ[1]
        meantau = Optim.minimizer(optimize(x->loss(x,J0=J0,End=posttau,y=y),max(t0,J0.τ[end]),t1))
        taum = rand(truncated(Normal(meantau,3.0),max(t0,J0.τ[end]),t1))
        llk += logpdf(truncated(Normal(meantau,3.0),max(t0,J0.τ[end]),t1),taum)
    else
        if J0.τ[end] <= t0
            taum = nothing
        else
...
function ProposedZDendity(Z,J0,t0,t1,t2,y,par,auxpar)
    ...
    if Z.M == 1
        # A birth is proposed, proposal a jumpt between J0.τ[end] and 
        posttau = Z.X.K==0 ? t2 : Z.X.τ[1]
        meantau = Optim.minimizer(optimize(x->loss(x,J0=J0,End=posttau,y=y),max(t0,J0.τ[end]),t1))
        llk += logpdf(truncated(Normal(meantau,3.0),max(t0,J0.τ[end]),t1),Z.taum)
    else
        if J0.τ[end] <= t0
            llk += 0.0
        else
